...
import { AffixGroupByAvailOn } from "./src/js/config/Affix.js";
import { Stats } from "./src/js/config/Stat.js"
import { objEntries } from './src/js/Utils.js';
import Const from "./src/js/Const.js"
import { execSync } from "child_process";

// generate affix map
function generateAffixMap() {
    const exclude = [
        "rthp",
        "rtmaxhp",
        "rtmp",
    ...
    /**@type {any}*/
    const result = {};
    for (const [itemType, configs] of objEntries(AffixGroupByAvailOn)) {
        for (const config of configs) {
            if (result[config.statId] == null) {
                result[config.statId] = {};
            }
            result[config.statId][itemType] = true;
        }
    }

    const rows = [];
    const header = ["名称"];
    const itemTypes = objEntries(Const.TYPE_NAME);
    for (const [typeId, _] of itemTypes) {
        header.push(`${typeId}`);
    }
    rows.push(header.join(","));

    for (const stat of Stats) {
        if (exclude.includes(stat.id)) {
            continue;
        }
        const row = [`${stat.name}(${stat.id})`];
        for (const [itemType, _] of itemTypes) {
            if (result[stat.id]?.[itemType]) {
                row.push("✔");
            } else {
                row.push("");
            }
    ...
    const csvPath = path.join(dataDir, "affix_map.csv");
    fs.writeFileSync(csvPath, "\uFEFF" + csvContent, { encoding: 'utf8' });
    console.log(`Affix map generated at ${csvPath}`);
    execSync(`start "" "${csvPath}"`);
}
try {
    generateAffixMap();
} catch (error) {
    console.log(`Error generating affix map: ${error.message}`);
}
